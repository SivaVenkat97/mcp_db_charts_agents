...
def store_sql_query_in_main_db(
    ...
    try:
        # Get session context from environment variables if not provided directly
        ...
        if conversation_id is None:
            conversation_id = os.getenv('MCP_CONVERSATION_ID')
            if conversation_id:
                try:
        ...
        logger.info(f"Stored SQL query in main DB: {query[:100]}... (ID: {sql_query.id}, Session: {session_id}, Conversation: {conversation_id})")
        return sql_query.id
